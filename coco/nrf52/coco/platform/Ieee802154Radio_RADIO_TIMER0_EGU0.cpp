...
void Ieee802154Radio_RADIO_TIMER0_EGU0::stop() {
	...
	for (auto &channel : this->nodes) {
		for (auto &buffer : channel.receiveBuffers) {
			buffer.p.transferred = 0;
			buffer.setState(Buffer::State::CANCELLED);
			buffer.setState(Buffer::State::READY);
		}
		for (auto &buffer : channel.requestBuffers) {
			buffer.p.transferred = 0;
			buffer.setState(Buffer::State::CANCELLED);
			buffer.setState(Buffer::State::READY);
		}
...
Ieee802154Radio_RADIO_TIMER0_EGU0::Buffer::Buffer(Node &node)
	: coco::Buffer(packet + 1, PACKET_LENGTH, State::READY), node(node)
{
...
bool Ieee802154Radio_RADIO_TIMER0_EGU0::Buffer::start(Op op, int size) {
	...
	if ((op & Op::READ) != 0) {
		...
		// send
		this->packet[0] = size - SEND_EXTRA_LENGTH + 2; // space for CRC
		auto sendFlags = SendFlags(this->packet[1 + size - SEND_EXTRA_LENGTH]);
...
void Ieee802154Radio_RADIO_TIMER0_EGU0::Buffer::cancel() {
	if (this->p.state == State::BUSY) {
		...
		if (this != this->node.radio.sendBuffer) {
			...
			// sending of this buffer is in progress
...
void Ieee802154Radio_RADIO_TIMER0_EGU0::backoff() {
	...
	if (this->backoffCount >= maxBackoffCount) {
		//setSendResult(0);
		this->sendBuffer->p.transferred = 0; // indicate failure
...
void Ieee802154Radio_RADIO_TIMER0_EGU0::handleRadio() {
	...
	if (NRF_RADIO->EVENTS_RXREADY) {
		...
		// start receive
		if (this->receiverEnabled/* && !Radio::receiveQueue.isFull()*/)
			startReceive(); // -> END
	...
	if (NRF_RADIO->EVENTS_CRCOK) {
		...
		int length = this->receivePacket[0];
		...
		if (this->sendState == SendState::AWAIT_ACK) {
			...
			if (frameType == ieee::FrameControl::TYPE_ACK && receivePacket[3] == this->sendBuffer->packet[3]) {
				...
				//setSendResult(this->backoffCount);
				this->sendBuffer->finished = true;
		...
		if (RECEIVE_EXTRA_LENGTH >= 5) {
			...
			uint8_t *ts = &this->receivePacket[1 + length - 2 + 1];
			ts[0] = timestamp;
			ts[1] = timestamp >> 8;
			ts[2] = timestamp >> 16;
			ts[3] = timestamp >> 24;
				...
				for (auto &buffer : node.receiveBuffers) {
					if (!buffer.finished) {
						...
						if (passThis) {
							...
							int l = length - 2 + RECEIVE_EXTRA_LENGTH;
							std::copy(this->receivePacket + 1, this->receivePacket + 1 + l,
								buffer.packet + 1);
							buffer.p.transferred = l;
...
void Ieee802154Radio_RADIO_TIMER0_EGU0::handleTimer() {
	...
	if (TEST(INTEN, TIMER_INTENSET_COMPARE3, Enabled) && NRF_TIMER0->EVENTS_COMPARE[3]) {
		...
		NRF_TIMER0->TASKS_CAPTURE[3] = TRIGGER;
		int t = NRF_TIMER0->CC[3] - NRF_TIMER0->CC[2];
