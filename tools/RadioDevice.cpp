...
static const UsbConfiguration configurationDescriptor = {
    .config = {
        ...
        .iConfiguration = 0,
        .bmAttributes = usb::ConfigurationDescriptorAttriubtes::BUS_POWERED,// 0x80, // bus powered
        .bMaxPower = 50 // 100 mA
    },
    .interface = {
        .bInterfaceNumber = 0,
    ...
    while (true) {
        ...
        while (radioBuffer.ready() && usbBuffer.ready()) {
            ...
            if (transferred == 1) {
                ...
                auto header = buffer;
                int headerSize = radioBuffer.headerCapacity();// data[0];
                auto data = header + headerSize;
                int size = transferred - /*1 -*/ headerSize;
                if (size >= 2) {
                    // get mac counter to identify the packet
                    uint8_t macCounter = data[2];

                    // set header
                    radioBuffer.setHeader(header, headerSize);
